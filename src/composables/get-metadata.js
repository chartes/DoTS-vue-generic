...
export default async function fetchMetadata(resourceId, documentType, route) {
    let startTimefetchMetadata = new Date();
    ...
    const getMetadata = async function () {
        ...
        if (dublincore) {
            ...
            if (Array.isArray(dublincore.creator)) {
                metadata.author = []
                for (let aut of dublincore.creator) {
        ...
        if (listmetadata.download) {
            ...
            if (listmetadata.download.XML) {
                ...
                if (source) {
                    metadata.downloadXML = {'source': source, 'url': listmetadata.download.XML}
        ...
        if (dublincore && extensions) {
            ...
            if (extensions["dct:isVersionOf"]) {
                ...
                for (let member of extensions["dct:isVersionOf"]) {
                    if (member["@id"]) {
                        const source = findSource(member["@id"]);
                        if (source) {
                            metadata["dct:isVersionOf"].push({'source': source, 'url': member["@id"]})
            ...
            if (extensions["dct:license"]) {
                ...
                if (source) {
                    metadata["dct:license"] = {'source': source, 'url': extensions["dct:license"]}
            ...
            if (Array.isArray(extensions["dct:creator"])) {
                ...
                for (let aut of extensions["dct:creator"]) {
                    if (aut["@id"]) {
                        ...
                        if (source) {
                            metadata["dct:creator"].push({'source': source, 'url': aut["@id"]})
    ...
    await getMetadata()
    let endTimefetchMetadata = new Date();
